 ...
 * to be cleaned.
 */
export async function getSavedSessionIds() {
  if (config.USE_POSTGRES) return [];
  const parsed = path.parse(config.SQLITE_PATH);
  const sessionsDir = path.join(parsed.dir || ".", "sessions");
  try {
    const files = await fs.readdir(sessionsDir);
    return files.filter((file) => file.endsWith(".db")).map((file) => file.slice(0, -3)).filter((id) => id !== "default");
  } catch {
    return [];
  }
}

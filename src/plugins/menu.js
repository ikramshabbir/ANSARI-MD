...
const DESCRIPTIONS = {
  ...
  repeat: "Repeat replied message 1–100 times",
  presence: "Choose Last Seen Mode",
...
async function buildHelpMenuText(showDescriptions = true) {
  ...
  // .repeat belongs ONLY to OWNER
  const repeatCmd = cmds.find(c => String(c.patternName || "").trim() === "repeat");
  if (repeatCmd) {
    for (const [type, list] of byType) {
      if (type !== "owner") {
        for (let i = list.length - 1; i >= 0; i--) {
          if (String(list[i].patternName || "").trim() === "repeat") {
            list.splice(i, 1);
          }
        }
      }
    ...
    const ownerList = byType.get("owner");
    if (ownerList && !ownerList.some(c => String(c.patternName || "").trim() === "repeat")) {
      ownerList.push(repeatCmd);
    }
  }

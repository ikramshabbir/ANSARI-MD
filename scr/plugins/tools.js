...
import { requireFfmpegPath } from "../utils/ffmpeg.js";
...
async function uploadCatbox(buffer, filename) {
  ...
  try {
    ...
    if (res.status >= 200 && res.status < 300 && body.startsWith("http")) {
      return body;
    }
  } catch {
    // Try next uploader
  ...
  try {
    ...
    if (
      ...
      return String(data.files[0].url);
    }
  } catch {
    // Continue to final error
  }

  throw new Error("File upload failed on all available upload services.");
}
    ...
    await withTyping(conn, message.from, async () => {
      ...
      try {
        ...
        const textFilters = lines.slice(0, 8).map((l, i) =>
          `drawtext=fontfile=${process.cwd()}/fonts/Roboto-Regular.ttf:text='${esc(l)}':fontcolor=white:fontsize=58:borderw=2:bordercolor=white:x=(w-text_w)/2:y=${125 + i * 55}`
        ).join(",");
        ...
        await runFFmpeg(requireFfmpegPath(), [
          "-hide_banner",
          "-loglevel", "error",
          "-f", "lavfi",
          "-i", "color=c=black:s=512x512:r=1",
          "-vf",
          `${textFilters},drawtext=fontfile=${process.cwd()}/fonts/Roboto-Regular.ttf:text='— ${safeName}':fontcolor=#d9d9d9:fontsize=28:borderw=1:bordercolor=#d9d9d9:x=w-text_w-28:y=h-text_h-24`,
          "-frames:v", "1",
          "-c:v", "libwebp",
          "-q:v", "75",
          "-y",
...
async function textToSticker(message, conn, { animated = false } = {}) {
  ...
  const font = `${process.cwd()}/fonts/Roboto-Regular.ttf`;
  if (!animated) {

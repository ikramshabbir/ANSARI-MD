...
const PRESENCE_START_DELAY_MS = 1000;
...
const activePairingSessions = new Set();
...
function stopPresence(sessionId) {
...
function startPresence(sessionId, conn) {
  stopPresence(sessionId);
  ...
  const sendAvailable = async () => {
    try {
      ...
      const mode = (await kvGet(`presence:${sessionId}`)) || "normal";
      ...
      console.log(
        `⚠️ Presence update failed [${sessionId}]:`,
        error?.message || error
      );
    }
  };
    ...
    try {
      ...
      conn.ev.on(
        ...
        async (update) => {
          ...
          if (qr) {
            const ready = pairingReadyPromises.get(sessionId);
          ...
          if (connection === "open") {
            ...
            /*
             * Start/maintain WhatsApp availability
             * whenever the socket becomes connected.
             *
             * This also runs again automatically after
          ...
          if (connection === "close") {
            /*
             * Stop the old socket's presence timer.
             * A new timer will automatically start after
             * the reconnecting socket reaches OPEN.
             */
            stopPresence(sessionId);
            ...
            if (
              ...
              activePairingSessions.has(sessionId)
            ) {
              console.log(
                `🔄 Pairing socket restart required: ${sessionId}`
              );

              setTimeout(() => {
                connect(sessionId).catch((error) => {
                  console.log(
                    `❌ Pairing restart failed: ${sessionId}`,
                    error?.message || error
                  );
                });
              }, 1000);
            ...
            if (
              activePairingSessions.has(
                sessionId
              )
            ) {
              console.log(
                `⏸️ Pairing session closed; automatic reconnect paused: ${sessionId}`
              );
              ...
              if (
                ...
                if (cacheKey) {
                  if (msgCache.has(cacheKey)) {
                    console.log(
                      `⏭️ DUPLICATE MESSAGE SKIPPED [${sessionId}]:`,
                      rawMessage.key.id
                    );
                    continue;
                  }
...
export async function requestPortalPairing(
  ...
  try {
    ...
    /*
     * Wait for Baileys to initialize pairing.
     */
    const ready =
          ...
          (_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error(
                    "WhatsApp pairing initialization timed out"
                  )
                ),
              15000
            )
    ...
    console.log(
      `🧪 Pairing socket state: connected=${conn?.ws?.isOpen ?? "unknown"}, user=${conn?.user ? "present" : "none"}`
    );

    activePairingSessions.add(sessionId);
